...
import com.algodal.game.rockpaperscissors.Constants;
	...
	public LateInitialization getLateInitialization() {
		return new LateInitialization() {
			...
			public void initialize(final SubGame sg) {
				...
				remAdsBtn = new TextButton("Advance Settings", (Skin) sg.get(defSkin)); remAdsBtn.setTransform(true);
				...
				remAdsBtn.addListener(new ClickListener() {
					@Override
					public void clicked(InputEvent event, float x, float y) {
						sg.setScene(Constants.advScene);
						sg.playTone();
					}
				});
